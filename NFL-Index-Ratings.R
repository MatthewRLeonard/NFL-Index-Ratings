...
games <- nflfastR::load_pbp(2014:2020, qs = FALSE)
...
team_stats <- team_stats %>%
  dplyr::mutate(scaled_epa_pp = scales::rescale(net_epa_pp, to = c(0, 0.2), from = range(net_epa_pp)),
         scaled_success_rate = rescale(net_success_rate, to = c(0, 0.2), from = range(net_success_rate)),
         scaled_yards_per_play = rescale(net_yards_per_play, to = c(0, 0.2), from = range(net_yards_per_play)),
         scaled_available_yards_perc = rescale(net_available_yards_perc, to = c(0, 0.2), from = range(net_available_yards_perc)),
         scaled_explosiveness = rescale(net_explosiveness, to = c(0, 0.1), from = range(net_explosiveness)),
         scaled_negatives = rescale(net_negatives, to = c(0, 0.1), from = range(net_negatives)),
         ...
         scaled_off_epa_pp = rescale(off_epa_pp, to = c(0, 0.2), from = range(off_epa_pp)),
         scaled_off_success_rate = rescale(off_success_rate, to = c(0, 0.2), from = range(off_success_rate)),
         scaled_off_yards_per_play = rescale(off_season_yards_per_play, to = c(0, 0.2), from = range(off_season_yards_per_play)),
         scaled_off_available_yards_perc = rescale(off_season_available_yards_perc, to = c(0, 0.2), from = range(off_season_available_yards_perc)),
         scaled_off_explosiveness = rescale(off_explosiveness, to = c(0, 0.1), from = range(off_explosiveness)),
         scaled_off_negatives = rescale(off_negatives, to = c(0.1, 0), from = range(off_negatives)),
         ...
         scaled_def_epa_pp = rescale(def_epa_pp, to = c(0.2, 0), from = range(def_epa_pp)),
         scaled_def_success_rate = rescale(def_success_rate, to = c(0.2, 0), from = range(def_success_rate)),
         scaled_def_yards_per_play = rescale(def_season_yards_per_play, to = c(0.2, 0), from = range(def_season_yards_per_play)),
         scaled_def_available_yards_perc = rescale(def_season_available_yards_perc, to = c(0.2, 0), from = range(def_season_available_yards_perc)),
         scaled_def_explosiveness = rescale(def_explosiveness, to = c(0.1, 0), from = range(def_explosiveness)),
         scaled_def_negatives = rescale(def_negatives, to = c(0, 0.1), from = range(def_negatives)),
